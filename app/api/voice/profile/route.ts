...
// GET — return stored profile (and post count)
export async function GET() {
  try {
    const [profile, posts] = await Promise.all([
      kv.get<VoiceProfile>("linkedin:voice_profile"),
      kv.get<LinkedInPost[]>("linkedin:posts"),
    ]);
...
// POST — (re)generate voice profile from stored posts
  ...
  try {
    ...
    const message = await anthropic.messages.create({
      ...
      messages: [
        {
          role: "user",
          content: `You are analysing LinkedIn posts by Eanna Barry, co-founder of Pair People (Sydney-based tech recruitment agency). Based on the ${sample.length} posts below, produce a voice profile as a JSON object.
...
{
  "writingStyle": "2–3 sentences describing her writing style",
  ...
  "summary": "2–3 paragraphs capturing Eanna's LinkedIn voice that can be injected into a writing prompt to make AI-generated posts match her style"
}`,
        },
      ],
    });
    ...
    await kv.set("linkedin:voice_profile", profile);
